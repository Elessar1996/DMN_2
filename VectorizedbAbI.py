...
class VectorizedbAbI:
    ...
    def __init__(self, data_directory='./data/en', task_ids=[1], batch_size=32):
        ...
        self.all_words = self.all_train_words + self.all_test_words


        self.vocab = build_vocab_from_iterator(
            self.get_tokens(self.all_words),
            min_freq=2,
        ...
        self.train_data_pipe = self.train_data_pipe.map(lambda data_instance: self.apply_transform(data_instance, self.vocab))
        self.test_data_pipe = self.test_data_pipe.map(lambda data_instance: self.apply_transform(data_instance, self.vocab))
