...
class FeatureVector(nn.Module):
    ...
    def __init__(self, embedding_size, middle_number):
        ...
        self.layer_1 = nn.Linear(in_features=self.size_of_scoring_network_layer,
                                 out_features=self.size_of_scoring_network_layer)
    ...
    def forward(self, c, m, q):
        ...
        z = torch.cat((flatten(c), flatten(m), flatten(q), flatten(torch.mul(c, q)), flatten(torch.mul(c, m)),
                       flatten(torch.abs(torch.sub(c, q))), flatten(torch.abs(torch.sub(c, m))), flatten(cwq),
                       flatten(cwm)), 1)
        return self.fnn(z)